...
switch($_GET['target']){
	...
	break;
	case "start":
		$result = shell_exec("ssh -i $installPath/id_rsa_npw -o StrictHostKeyChecking=no admin@localhost \"/etc/init.d/dnsmasq.sh start\"");
	...
	break;
	case "stop":
		$result = shell_exec("ssh -i $installPath/id_rsa_npw -o StrictHostKeyChecking=no admin@localhost \"/etc/init.d/dnsmasq.sh stop\"");
	...
	break;
	case "config":
		$file = "#### file generated by postdat.php ####\n\n";
...
EOF;
		...
		if($_POST['dns_server1']){
			$file .= "server=".$_POST['dns_server1']."\n";
		} else {
			$file .= "server=8.8.8.8\n";
		}
		if($_POST['dns_server2']){
			$file .= "server=".$_POST['dns_server2']."\n";
		} else {
			$file .= "server=8.8.4.4\n";
		}
		$file .= "\n";
